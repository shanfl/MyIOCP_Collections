...
void main()
{
	...
	dwThreadNum = GetPrivateProfileInt("config", "ThreadNum", 0, CONFIG_FILE);
	dwSendTick = GetPrivateProfileInt("config", "SendTick", 0, CONFIG_FILE);
	dwPackNum = GetPrivateProfileInt("config", "PackNum", 0, CONFIG_FILE);
	dwPackSize = GetPrivateProfileInt("config", "PackSize", 0, CONFIG_FILE);
	wDisProb = GetPrivateProfileInt("config", "DisProb", 0, CONFIG_FILE);
...
//end:
	//while(!pClientList.empty())
	//{
	//	delete pClientList.back();
	//	pClientList.pop_back();
	//}
	//printf("press enter key to exit");
	//getchar();
}
